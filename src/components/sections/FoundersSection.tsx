      ...
      <div className="container-custom relative z-10">
        ...
        >
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold font-display text-foreground mb-4">
            <EditableText contentKey="founders.heading" defaultValue="Meet Our Founders" />
          </h2>
          <EditableText
            contentKey="founders.description"
            defaultValue="The visionaries behind Hacker's Unity who are dedicated to empowering the tech community"
            as="p"
                ...
                <div className="relative mx-auto mb-8 w-40 h-40 md:w-48 md:h-48">
                  ...
                  <div className="absolute -bottom-4 left-1/2 -translate-x-1/2 z-10">
                    <span className="bg-primary text-primary-foreground px-4 py-1.5 rounded-full text-sm font-semibold shadow-lg whitespace-nowrap">
                      <EditableText contentKey={`founders.${index}.role`} defaultValue={founder.role} />
                    </span>
                  </div>
                </div>

                <EditableText
                  contentKey={`founders.${index}.name`}
                  defaultValue={founder.name}
                  as="h3"
                  className="text-xl md:text-2xl font-bold font-display text-foreground mt-4 mb-3"
                />

                <EditableText
                  contentKey={`founders.${index}.description`}
                  defaultValue={founder.description}
                  as="p"
                  className="text-muted-foreground text-sm md:text-base leading-relaxed mb-6 px-2"
                />

                <div className="flex justify-center gap-4">
                  <EditableLink contentKey={`founders.${index}.linkedin`} defaultHref={founder.socials.linkedin}>
                    {(href, editProps) => (
                      <a href={href} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-muted/50 border border-border flex items-center justify-center text-muted-foreground hover:bg-primary hover:text-primary-foreground hover:border-primary transition-all duration-300" aria-label={`${founder.name}'s LinkedIn`} {...editProps}>
                        <Linkedin className="w-5 h-5" />
                      </a>
                    )}
                  </EditableLink>
                  <EditableLink contentKey={`founders.${index}.x`} defaultHref={founder.socials.x}>
                    {(href, editProps) => (
                      <a href={href} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-muted/50 border border-border flex items-center justify-center text-muted-foreground hover:bg-primary hover:text-primary-foreground hover:border-primary transition-all duration-300" aria-label={`${founder.name}'s X`} {...editProps}>
                        <XIcon className="w-5 h-5" />
                      </a>
                    )}
                  </EditableLink>
                  <EditableLink contentKey={`founders.${index}.instagram`} defaultHref={founder.socials.instagram}>
                    {(href, editProps) => (
                      <a href={href} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-muted/50 border border-border flex items-center justify-center text-muted-foreground hover:bg-primary hover:text-primary-foreground hover:border-primary transition-all duration-300" aria-label={`${founder.name}'s Instagram`} {...editProps}>
