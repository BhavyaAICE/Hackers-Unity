...
const EventForm = () => {
  ...
  const fetchEvent = async () => {
    try {
      const { event: data } = await api.get<{ event: any }>(`/api/events/${id}`);
      setFormData({
        title: data.title || "",
        subtitle: data.subtitle || "",
        description: data.description || "",
        event_type: data.eventType || "conference",
        ...
        benefits_text: data.benefitsText || "",
        external_link: data.externalLink || "",
      });
      if (data.speakers) {
        setSpeakers(data.speakers.map((s: any) => ({ name: s.name, title: s.title, avatar_url: s.avatarUrl, linkedin_url: s.linkedinUrl })));
        setSpeakerAltTexts(data.speakers.map((s: any) => s.name));
      }
      if (data.schedule) {
        setScheduleItems(data.schedule.map((s: any) => ({
          day: s.day,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        benefitsText: formData.benefits_text || null,
        externalLink: formData.external_link || null,
        speakers: speakers.filter(s => s.name).map((s, i) => ({
          name: s.name,
          title: s.title || null,
        ...
        schedule: scheduleItems.filter(s => s.title).map((s, i) => ({
          day: s.day,
