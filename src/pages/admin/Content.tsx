...
const Content = () => {
  ...
  const fetchContent = async () => {
    try {
      const { content } = await api.get<{ content: { id: string; content: HeroContent } | null }>("/api/content/hero");
      if (content) {
        setContentId(content.id);
        setHeroContent({
          ...defaultHeroContent,
          ...content.content,
          images: content.content?.images || defaultHeroContent.images,
        });
      }
    } catch (error: any) {
  ...
  const handleSave = async () => {
    ...
    try {
      if (contentId) {
        await api.put(`/api/content/hero/${contentId}`, { content: heroContent });
      } else {
        const result = await api.post<{ id: string }>("/api/content/hero", { content: heroContent });
