  ...
  const fetchHackathon = async () => {
    ...
    try {
      ...
      setFormData({
        ...
        event_date: data.eventDate ? new Date(data.eventDate).toISOString().slice(0, 16) : "",
        event_end_date: data.eventEndDate ? new Date(data.eventEndDate).toISOString().slice(0, 16) : "",
        registration_deadline: data.registrationDeadline ? new Date(data.registrationDeadline).toISOString().slice(0, 16) : "",
        duration_days: data.durationDays || 1,
        timing: data.timing || "",
        location_type: data.locationType || "offline",
        location_name: data.locationName || "",
        location_address: data.locationAddress || "",
        banner_image: data.bannerImage || "",
        thumbnail_image: data.thumbnailImage || "",
        registration_enabled: data.registrationEnabled ?? true,
        prize_pool: data.prizePool || "",
        max_participants: data.maxParticipants?.toString() || "",
        external_link: data.externalLink || "",
      });
      setChallenges((data.challenges || []).map((c: any) => ({ ...c, image_alt: `${c.title} challenge` })));
      setMentors((data.mentors || []).map((m: any) => ({ ...m, avatar_alt: `${m.name} photo` })));
      setJury((data.jury || []).map((j: any) => ({ ...j, avatar_alt: `${j.name} photo` })));
      setPrizes(data.prizes || []);
      setFaqs(data.faqs || []);
      setWinners((data.winners || []).map((w: any) => ({ ...w, image_alt: `${w.teamName || w.team_name} project` })));
      setSchedule((data.schedule || []).map((s: any) => ({ ...s, schedule_date: s.scheduleDate || "" })));
    } catch {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        eventEndDate: formData.event_end_date || null,
        registrationDeadline: formData.registration_deadline || null,
        durationDays: formData.duration_days,
        timing: formData.timing || null,
        locationType: formData.location_type,
        locationName: formData.location_name || null,
        locationAddress: formData.location_address || null,
        bannerImage: formData.banner_image || null,
        thumbnailImage: formData.thumbnail_image || null,
        registrationEnabled: formData.registration_enabled,
        prizePool: formData.prize_pool || null,
        maxParticipants: formData.max_participants ? parseInt(formData.max_participants) : null,
        externalLink: formData.external_link || null,
        challenges: challenges.filter(c => c.title).map((c, i) => ({
          title: c.title, description: c.description || null, detailedDescription: c.detailed_description || null,
          icon: c.icon || null, imageUrl: c.image_url || null, isActive: c.is_active, displayOrder: i,
        })),
        mentors: mentors.filter(m => m.name).map((m, i) => ({
          name: m.name, title: m.title || null, organization: m.organization || null, avatarUrl: m.avatar_url || null,
          linkedinUrl: m.linkedin_url || null, bio: m.bio || null, isActive: m.is_active, displayOrder: i,
        })),
        jury: jury.filter(j => j.name).map((j, i) => ({
          name: j.name, title: j.title || null, organization: j.organization || null, avatarUrl: j.avatar_url || null,
          linkedinUrl: j.linkedin_url || null, bio: j.bio || null, isActive: j.is_active, displayOrder: i,
        })),
        prizes: prizes.filter(p => p.position).map((p, i) => ({
          position: p.position, title: p.title || null, prizeAmount: p.prize_amount || null,
          description: p.description || null, iconUrl: p.icon_url || null, isActive: p.is_active, displayOrder: i,
        })),
        faqs: faqs.filter(f => f.question).map((f, i) => ({
          question: f.question, answer: f.answer, isActive: f.is_active, displayOrder: i,
        })),
        winners: winners.filter(w => w.team_name).map((w, i) => ({
          position: w.position, teamName: w.team_name, projectName: w.project_name,
          projectDescription: w.project_description || null, members: w.members || null,
          prizeWon: w.prize_won || null, imageUrl: w.image_url || null,
          githubUrl: w.github_url || null, projectUrl: w.project_url || null, displayOrder: i,
        })),
        schedule: schedule.filter(s => s.title).map((s, i) => ({
          day: s.day, scheduleDate: s.schedule_date || null, startTime: s.start_time || null,
          endTime: s.end_time || null, title: s.title, description: s.description || null, displayOrder: i,
        })),
      };
